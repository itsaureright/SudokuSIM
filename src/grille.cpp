...
vector<suint> Grille::valeursAdmissiblesDiag(suint i, suint j)
{
    vl.clear(); vc.clear(); vb.clear(); vs.clear();
    vector<suint> vd; // valeurs interdites par les diagonales
    ...
    if (i == j) {
        for (suint k = 0; k < N; ++k) {
            if (grille[k][k] != 0)
                vd.push_back(grille[k][k]);
        }
    }

    // Diagonale secondaire (i + j == N - 1)
    if (i + j == N - 1) {
        for (suint k = 0; k < N; ++k) {
            if (grille[k][N - k - 1] != 0)
                vd.push_back(grille[k][N - k - 1]);
        }
    }
    ...
    for (suint v = 1; v <= N; ++v) {
        ...
        for (auto x : vc) if (x == v) interdit = true;
        for (auto x : vb) if (x == v) interdit = true;
        for (auto x : vd) if (x == v) interdit = true;
...
void Grille::genererGrillePartielleDiag(suint p)
{   // remise à zéro
    suint N = n*n ;
    for (suint i = 0; i < N; ++i)
        for (suint j = 0; j < N; ++j)
    ...
    while (nbRemplies < p && !casesVides.empty())
    {
        ...
        // valeurs admissibles
        vector<suint> admissibles = valeursAdmissiblesDiag(i, j);
